...
class TrucksState:
    ...
    def __init__(self, timestep):
        ...
        self.cross_e_cost = 0  # Cost of Cross_E (empty teleport)
        self.cross_k_cost = 0  # Cost of Cross_K (loaded move)
...
class TrucksVisualizer:
    ...
    def _parse_dzn(self):
        ...
        # Extract weights matrix (commented in .dzn file)
        match = re.search(r'%\s*Weights\s*=\s*\[\|([^\]]+)\|\]', content, re.DOTALL)
        if match:
            matrix_str = match.group(1)
            rows = [line.strip()
                    for line in matrix_str.split('|') if line.strip()]
            ...
            for row in rows:
                ...
                if row_clean:
                    values = [int(x.strip())
                              for x in row_clean.split(',') if x.strip()]
                    self.weights_matrix.append(values)
            self.weights_matrix = np.array(self.weights_matrix)
    ...
    def _parse_output(self, output):
        ...
        while i < len(lines):
            ...
            if match:
                ...
                if i + 2 < len(lines):
                    # Cross_E cost
                    cross_e_line = lines[i + 2]
                    match = re.search(r'Cross_E cost:\s*(\d+)', cross_e_line)
                    if match:
                        state.cross_e_cost = int(match.group(1))
                ...
                if i + 4 < len(lines):
                    # Cross_K cost
                    cross_k_line = lines[i + 4]
                    match = re.search(r'Cross_K cost:\s*(\d+)', cross_k_line)
                    if match:
                        state.cross_k_cost = int(match.group(1))

                if i + 5 < len(lines):
                    # Truck position and contents
                    truck_line = lines[i + 5]
                    match = re.search(
    ...
    def _draw_state(self, state):
            ...
            for i in range(self.N):
                for j in range(i + 1, self.N):
                    if self.adj_matrix[i][j] == 1:
                        ...
                        if weight > 0:
                            edge_labels[(i + 1, j + 1)] = str(weight)
        ...
        if state.timestep > 0 and state.move_to:
            ...
            if self.G.has_edge(load_pos, state.move_to):
                ...
                # Add cost label for Cross_K
                mid_x, mid_y = (x1 + x2) / 2, (y1 + y2) / 2
                self.ax.text(mid_x, mid_y - 0.1, f'cost: {state.cross_k_cost}',
                             ha='center', va='top', fontsize=9,
                             color='#0066FF', weight='bold',
                             bbox=dict(boxstyle='round,pad=0.3',
                                       facecolor='white', alpha=0.9,
                                       edgecolor='#0066FF', linewidth=1.5))
        ...
        if state.timestep > 0 and state.had_cross_e == 1:
            ...
            if prev_state.truck_pos != state.load_from:
                ...
                # Add label with cost
                mid_x, mid_y = (x1 + x2) / 2, (y1 + y2) / 2
                label_text = f'Cross_E\ncost: {state.cross_e_cost}'
                self.ax.text(mid_x, mid_y + 0.1, label_text,
        ...
        if state.timestep > 0:
            ...
            cost_info = f'Cross_E cost: {state.cross_e_cost}, Cross_K cost: {state.cross_k_cost}'
            title += f'\n({cost_info})'
        else:
            title += ' (Initial State)'
        ...
        # Calculate cumulative costs up to current timestep
        if state.timestep > 0:
            total_cross_e = sum(s.cross_e_cost for s in self.states[1:state.timestep+1])
            total_cross_k = sum(s.cross_k_cost for s in self.states[1:state.timestep+1])
            total_cost = total_cross_e + total_cross_k
            
            # Add cost summary box in bottom right
            cost_text = f'Cumulative Costs (up to t={state.timestep}):\n'
            cost_text += f'Total Cross_E: {total_cross_e}\n'
            cost_text += f'Total Cross_K: {total_cross_k}\n'
            cost_text += f'TOTAL: {total_cost}'
            
            self.ax.text(0.98, 0.02, cost_text,
        ...
        legend_elements = [
            ...
            plt.Line2D([0], [0], color='#0066FF',
                       linewidth=3, label='Cross_K (loaded move)'),
            plt.Line2D([0], [0], color='red', linestyle='--',
                       linewidth=3, label='Cross_E (empty teleport)')
        ]
