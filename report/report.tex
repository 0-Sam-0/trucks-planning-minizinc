...
\begin{titlepage}
    ...
    {\hypersetup{urlcolor=black}%
     \href{https://github.com/0-Sam-0/trucks-planning-minizinc}%
          {\small\emph{Models, benchmark data and errata --- revised after submission:}}\par
     \vspace{0.15cm}
     \href{https://github.com/0-Sam-0/trucks-planning-minizinc}%
          {\small\texttt{github.com/0-Sam-0/trucks-planning-minizinc}}\par}
...
\begin{table}[H]
    \centering
    \caption{Solver performance comparison on base problem (100 instances, 300s timeout). Figures in parentheses are averages over the instances that solver solved.}
...
The results reveal a somewhat surprising conclusion: Chuffed's default search strategy beats every one of the manually designed alternatives. Sequential-timestep ordering, position-first variable selection and adaptive domain splitting are all slower than the default on the mean, by factors ranging from 1.3 to 12.2, and the best of the twelve improves on the default on only 7 of the 33 instances in this subset. This finding suggests that Chuffed's internal heuristics and lazy clause generation mechanism are already well-tuned for this problem structure. The solver automatically learns and exploits patterns during search, reducing the need for hand-crafted annotations. For practitioners, this means the default configuration is a robust starting point, eliminating the time-consuming process of strategy tuning.

\subsubsection{Scaling Behavior}

An analysis of runtimes across different graph sizes reveals that performance degrades as the problem scale increases. Instances with N=10 nodes typically solve in under 2 seconds with Chuffed, while instances with N=26 nodes average 2.57 seconds and never exceed 3.8 seconds; the slowest base instance overall, at 6.50 seconds, occurs at N=22. The timeout instances for Gecode and HiGHs are predominantly concentrated in the larger graph sizes (N $\ge$ 18), confirming that Chuffed's lazy clause generation provides a distinct advantage on harder, more structured instances.
...
Phase 1 is always run with Chuffed, whichever solver is being measured in Phase 2. This keeps $k_{min}$ identical across runs, so every solver is given the same problem to optimize; it also means the Phase 1 time is the same work in every run and says nothing about the solver under test.
...
The Solved column counts the instances for which the solver returned a plan with a cost, whether or not it went on to prove that cost optimal. The Optimal column counts the subset it did prove optimal, and is therefore never larger. On the remaining instances the solver returned nothing at all before the 300-second limit expired; whether it was still searching or had exhausted the available memory is not recorded. Every instance in the benchmark does admit a plan, so these are failures of the solver, not infeasible problems.

\begin{table}[H]
    \centering
    \caption{Solver performance on minimization problem (100 instances, 300s timeout per phase). Figures in parentheses are averages over the instances that solver proved optimal.}
    \label{tab:min-solver-stats}
    \begin{tabular}{|l|c|c|c|c|c|}
        \hline
        \textbf{Solver} & \textbf{Optimal} & \textbf{Solved} & \textbf{Avg Cost} & \textbf{Min. phase (s)} & \textbf{Total (s)} \\
        \hline
        Chuffed         & 100/100          & 100/100           & 50.5              & 2.52             & 3.86               \\
        Gecode          & 82/100           & 92/100            & 47.9              & 61.54 (9.17)     & 62.93 (10.20)      \\
        HiGHs           & 67/100           & 91/100            & 50.3              & 128.89 (44.50)   & 130.19 (45.36)     \\
        OR-Tools        & 92/100           & 92/100            & 47.5              & 38.33 (15.56)    & 39.63 (16.61)      \\
        \hline
    \end{tabular}
\end{table}

The Min. phase column reports the cost minimization alone; the Total column adds
the satisfiability search that precedes it, which costs about 1.3 seconds per
instance. As noted in Section~\ref{sec:two-phase}, that search is always run with
Chuffed, so it contributes the same work to every row and only the Min. phase
column compares the solvers. Failed instances are counted at the runtime measured for
them. The analysis below refers to the minimization phase.

Chuffed again demonstrates exceptional performance, achieving 100\% proven optimality (100 solved, 100 optimal) with an average minimization phase runtime of just 2.52 seconds and no failures. This is significantly faster than all other solvers: approximately 3.6 times faster than Gecode's proven optimal instances average (9.17s), 17.7 times faster than HiGHs' proven optimal instances (44.50s), and 6.2 times faster than OR-Tools' proven optimal instances (15.56s). The optimization phase proves that Chuffed's lazy clause generation excels not only at finding feasible solutions but also at proving cost optimality efficiently.

Gecode found solutions for 92 instances but proved optimality for only 82, failing to return anything on 8 and leaving 10 with potentially suboptimal solutions (where a solution was found but optimality could not be proven within the timeout). Its average MIN phase time is 61.54 seconds including failed instances (9.17 seconds for proven optimal instances), still nearly 4 times slower than Chuffed even on successful cases. HiGHs struggles significantly on this optimization problem, finding solutions for 91 instances but proving optimality for only 67, failing on 9 and leaving 24 potentially suboptimal solutions. Its average MIN time is 128.89 seconds including failed instances (44.50 seconds for proven optimal instances). OR-Tools achieves a 92\% proven optimality rate (92 solved, all proven optimal, having failed on the remaining 8) with an average MIN time of 38.33 seconds including failed instances (15.56 seconds for proven optimal instances), but remains substantially slower than Chuffed.

\subsubsection{Cost Quality}

An interesting observation is the variation in average costs across solvers. The solutions found by Chuffed have an average cost of 50.5, while Gecode's average is 47.9, HiGHs' is 50.3, and OR-Tools' is 47.5. This discrepancy arises because the averages are computed over the solved instances for each solver, different subsets of the 100 instances where a solution was found. Importantly, for Gecode and HiGHs, these averages include solutions that were never proven optimal. Any such solution is an upper bound on the true optimum, so it can only raise an average, never lower it.

Gecode (92 solved instances, 82 proven optimal) and OR-Tools (92 solved instances, 92 proven optimal) both show lower average costs than Chuffed (47.9 and 47.5 vs 50.5). For OR-Tools, since every solution it found is proven optimal, the comparison is direct: it solved a subset of instances (missing the 8 it failed on) that happen to have inherently lower optimal costs. For Gecode, the average includes 10 solutions it never proved optimal, which can only push its figure up, so its true advantage on that subset is if anything larger than 47.9 suggests. Comparing those unproven solutions against the optima Chuffed proved on the same instances, 3 of Gecode's 10 are worse and 7 are equal, for a mean 6\% above the optimum. HiGHs (91 solved instances, 67 optimal) has an average cost of 50.3, nearly identical to Chuffed's, but its 24 unproven solutions are not close to optimal: 18 of them are strictly worse, and the mean is 16\% above the true optimum. Its apparent parity with Chuffed comes from the easier subset it managed to solve, not from the quality of its answers.

\subsubsection{Runtime Breakdown}

Figure~\ref{fig:min-solver-comparison} visualizes the runtime distribution for the minimization phase across all four solvers, including timeout instances (marked above the 300-second line).
...
The plot highlights Chuffed's tight runtime distribution, with 94 of 100 instances solving in under 5 seconds, a maximum of 83.44 seconds, and proven optimality throughout. Gecode failed on 8 instances and found 10 potentially suboptimal solutions where optimality could not be proven within the timeout (82\% proven optimal), OR-Tools failed on 8 with every solved instance achieving proven optimality (92\% proven optimal), and HiGHs failed on 9 and left 24 potentially suboptimal solutions where optimality could not be proven (67\% proven optimal).
...
The outcome repeats, more starkly: all ten annotated strategies are slower than the default on the mean, by factors from 1.8 to 14.5, and the best of them wins on 5 of the 20 instances in this subset. The optimization objective does not benefit from manual search annotations here.

\subsubsection{Minimization Phase Performance Summary}

The minimization phase results clearly demonstrate Chuffed's superiority in proving cost optimality. With an average MIN time of 2.52 seconds and a 100\% proven optimality rate, Chuffed significantly outperforms all other solvers. When including failed instances, counted at the runtime recorded for them, Gecode averages 61.54 seconds (82\% proven optimal), HiGHs averages 128.89 seconds (67\% proven optimal), and OR-Tools averages 38.33 seconds (92\% proven optimal).

Even when comparing only instances where optimality was proven, which provides a more favorable view of the competing solvers, Chuffed remains the clear winner: Gecode's 9.17 seconds is 3.6× slower, HiGHs' 44.50 seconds is 17.7× slower, and OR-Tools' 15.56 seconds is 6.2× slower than Chuffed's 2.52 seconds. This performance advantage, combined with the perfect proven optimality rate and no failures, confirms that Chuffed's lazy clause generation mechanism is exceptionally well-suited for optimization problems with complex temporal and resource constraints.
...
\begin{itemize}
    ...
    \item \textbf{OR-Tools:} 100\% success on BASE (3.96s avg), 92\% optimality on MIN (15.56s avg)
    \item \textbf{Gecode:} 86\% success on BASE (13.36s avg), 82\% optimality on MIN (9.17s avg)
    \item \textbf{HiGHs:} 75\% success on BASE (24.93s avg), 67\% optimality on MIN (44.50s avg)
\end{itemize}
